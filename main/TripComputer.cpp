...
TripComputer::TripComputer() : 
    ...
    _trip_avg_l_100km(0.0f), _trip_avg_kmh(0), _autonomy_km(0), _smoothed_fuel_level(-1.0f) {
}
...
void TripComputer::update(uint32_t dt_ms, uint16_t speed_kmh, float fuel_lh, uint8_t fuel_level) {
    if (dt_ms == 0) return;
    ...
    if (_recent_avg_l_100km > 0.5f && _smoothed_fuel_level > 0.0f) {
        _autonomy_km = (uint16_t)((_smoothed_fuel_level / _recent_avg_l_100km) * 100.0f);
    } else {
        _autonomy_km = 0;
    }
}
...
uint16_t TripComputer::getAutonomyKm() const { return _autonomy_km; }
